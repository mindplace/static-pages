...
(function(Opal) {
  ...
  Opal.add_stubs(['$require', '$append_path', '$untaint', '$expand_path', '$attr_reader', '$map', '$to_proc', '$p', '$score', '$each', '$<<', '$[]', '$keys', '$select', '$include?', '$sort', '$answers', '$==', '$max', '$values', '$new']);
  self.$require("opal");
  $scope.get('Opal').$append_path($scope.get('File').$expand_path("..", "ruby-to-js").$untaint());
  (function($base, $super) {
    function $BeverageQuiz(){};
    var self = $BeverageQuiz = $klass($base, $super, 'BeverageQuiz', $BeverageQuiz);

    var def = self.$$proto, $scope = self.$$scope;

    self.$attr_reader("answers");

    Opal.cdecl($scope, 'TEAS', $hash2(["earl_grey", "green", "herbal"], {"earl_grey": ["blue", "1", "sheep"], "green": ["green", "2", "dog"], "herbal": ["red", "3", "cat"]}));
    ...
    Opal.defn(self, '$initialize', function() {
      ...
      var array_size = arguments.length - 0;
      if(array_size < 0) array_size = 0;
      var answers = new Array(array_size);
      for($splat_index = 0; $splat_index < array_size; $splat_index++) {
        answers[$splat_index] = arguments[$splat_index + 0];
      }
      self.answers = ($a = ($b = answers).$map, $a.$$p = "to_s".$to_proc(), $a).call($b);
      return self.$p(self.$score());
    });

    return (Opal.defn(self, '$score', function() {
      var $a, $b, TMP_1, $c, TMP_3, self = this, score = nil, modes = nil;

      score = $hash2(["earl_grey", "green", "herbal"], {"earl_grey": [], "green": [], "herbal": []});
      ($a = ($b = self.$answers().$sort()).$each, $a.$$p = (TMP_1 = function(item){var self = TMP_1.$$s || this, $a, $b, TMP_2;
if (item == null) item = nil;
      return score['$[]'](($a = ($b = $scope.get('TEAS')).$select, $a.$$p = (TMP_2 = function(k, v){var self = TMP_2.$$s || this;
if (k == null) k = nil;if (v == null) v = nil;
        return v['$include?'](item)}, TMP_2.$$s = self, TMP_2), $a).call($b).$keys()['$[]'](0))['$<<'](item)}, TMP_1.$$s = self, TMP_1), $a).call($b);
      modes = ($a = ($c = score).$select, $a.$$p = (TMP_3 = function(k, v){var self = TMP_3.$$s || this;
if (k == null) k = nil;if (v == null) v = nil;
      return v['$=='](score.$values().$max())}, TMP_3.$$s = self, TMP_3), $a).call($c);
      return "Your best tea match is " + (modes.$keys()['$[]'](0)) + " tea!";
    }), nil) && 'score';
  })($scope.base, null);
  return $scope.get('BeverageQuiz').$new($scope.get('ARGV'));
})(Opal);
